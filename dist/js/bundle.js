...
var Game = (function () {
    ...
    Game.prototype.setup = function () {
        this.finished = false;
        for (var index134 = this.creatures.iterator(); index134.hasNext();) {
            var c = index134.next();
            {
        ...
        for (var index135 = this.creatures.iterator(); index135.hasNext();) {
            var c = index135.next();
            {
    ...
    Game.prototype.loop = function () {
        ...
        this.env.setCameraPitch(this.pitch);
        for (var index136 = this.creatures.iterator(); index136.hasNext();) {
            var c = index136.next();
            {
                c.move(this.creatures, this.dead_creatures);
            }
        }
        for (var index137 = this.dead_creatures.iterator(); index137.hasNext();) {
            var c = index137.next();
            {
                this.env.removeObject(c);
        ...
        var hasTux = false;
        for (var index138 = this.creatures.iterator(); index138.hasNext();) {
            var c = index138.next();
            {
...
var Fox = (function (_super) {
    ...
    Fox.prototype.move = function (creatures, dead_creatures) {
        for (var index139 = creatures.iterator(); index139.hasNext();) {
            var c = index139.next();
            {
        ...
        for (var index140 = creatures.iterator(); index140.hasNext();) {
            var c = index140.next();
            {
                if (c != null && c instanceof Tux) {
